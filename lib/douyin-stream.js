...
const QUALITY_KEY_PREFERENCE = {
  origin: ['ORIGIN', 'FULL_HD1', 'HD1', 'SD1', 'SD2'],
  hd: ['HD1', 'FULL_HD1', 'ORIGIN', 'SD1', 'SD2'],
...
// 源流(主播推流端)标称参数。sdk_params 是编码端配置：分辨率/码率/帧率/编码，
// 与观看端拉哪一档、本地带宽/解码性能完全无关 —— 监控要展示的是这个，不是本地实测值。
function parseSdkParams(str) {
  const meta = {};
  try {
    const p = JSON.parse(str || '{}');
    const res = String(p.resolution || '');
    const m = /^(\d+)x(\d+)$/.exec(res);
    if (m) { meta.w = Number(m[1]); meta.h = Number(m[2]); }
    if (Number(p.vbitrate) > 0) meta.kbps = Math.round(Number(p.vbitrate) / 1000);
    if (Number(p.fps) > 0) meta.fps = Math.round(Number(p.fps));
    if (p.VCodec) meta.codec = String(p.VCodec);
  } catch { /* 无 sdk_params 就空着，前端有兜底 */ }
  return meta;
}

// 从 stream_data 提取源流(origin=主播真实推流)的标称参数；
// qualities 列表兜底补缺（取 level 最高一档，即最接近推流源的档位）。
function extractSrcMeta(streamUrl) {
  let meta = {};
  try {
    const su = streamUrl || {};
    let streamDataStr = '';
    if (su.pull_datas && Object.keys(su.pull_datas).length) {
      streamDataStr = su.pull_datas[Object.keys(su.pull_datas)[0]].stream_data;
    } else if (su.live_core_sdk_data && su.live_core_sdk_data.pull_data) {
      streamDataStr = su.live_core_sdk_data.pull_data.stream_data;
    }
    if (streamDataStr) {
      const sd = JSON.parse(streamDataStr);
      const originMain = sd && sd.data && sd.data.origin && sd.data.origin.main;
      if (originMain) meta = parseSdkParams(originMain.sdk_params);
    }
    if ((!meta.kbps || !meta.w || !meta.fps)
      && su.live_core_sdk_data && su.live_core_sdk_data.pull_data
      && su.live_core_sdk_data.pull_data.options
      && Array.isArray(su.live_core_sdk_data.pull_data.options.qualities)) {
      const qs = su.live_core_sdk_data.pull_data.options.qualities;
      const best = qs.reduce((a, b) => (!a || (Number(b.level) || 0) > (Number(a.level) || 0)) ? b : a, null);
      if (best) {
        if (!meta.w) {
          const m = /^(\d+)[x×](\d+)$/.exec(String(best.resolution || ''));
          if (m) { meta.w = Number(m[1]); meta.h = Number(m[2]); }
        }
        if (!meta.kbps && Number(best.v_bit_rate) > 0) meta.kbps = Math.round(Number(best.v_bit_rate) / 1000);
        if (!meta.fps && Number(best.fps) > 0) meta.fps = Math.round(Number(best.fps));
        if (!meta.codec && best.v_codec) meta.codec = String(best.v_codec);
      }
    }
  } catch { /* 结构变了也不影响取流 */ }
  return meta;
...
function parseEnterPayload(payload) {
  ...
  // 抖音 room.status：2=直播中，4=已结束。
  // 关键：只有明确 ===4 才算真下播；其它一切(缺字段/风控残缺/中间态/0/undefined)判 unknown，
  // 绝不因为"不等于2"就误判下播 —— 这是"在播却显示下播"批量误报的根因。
  let liveStatus;
  if (room.status === 2) liveStatus = 'live';
  else if (room.status === 4) liveStatus = 'ended';
  else liveStatus = 'unknown';
  if (liveStatus !== 'live' || !room.stream_url) {
    return {
      ok: false,
      // 在播但暂时没拿到 stream_url、或状态不确定 → 一律 unknown，让兜底抓取/前端重连去确认，不轻判下播
      status: liveStatus === 'ended' ? 'ended' : 'unknown',
      anchorName: user.nickname || '',
      title: room.title || '',
      flv: {},
      hls: {},
      userCount: '',
      reason: room.stream_url ? `room-status:${room.status}` : 'no-stream-url',
    };
  }
  const { flv, hls } = normalizeStreamUrls(room.stream_url);
  // 真·原画：藏在 live_core_sdk_data 里的单独更高码率流（flv_pull_url 里的 FULL_HD1 只是蓝光，不是原画）
  try {
    ...
    if (streamDataStr) {
      const sd = JSON.parse(streamDataStr);
      const originMain = sd && sd.data && sd.data.origin && sd.data.origin.main;
      if (originMain && originMain.flv) {
        let codec = '';
        try { codec = (JSON.parse(originMain.sdk_params || '{}').VCodec) || ''; } catch {}
        const suffix = codec ? (originMain.flv.includes('?') ? '&' : '?') + 'codec=' + codec : '';
        flv.ORIGIN = originMain.flv + suffix;
        if (originMain.hls) hls.ORIGIN = originMain.hls + (codec ? (originMain.hls.includes('?') ? '&' : '?') + 'codec=' + codec : '');
      }
    }
  } catch { /* 没有原画就用 flv_pull_url 里的 */ }
  const userCount =
  ...
  return {
    ...
    srcMeta: extractSrcMeta(room.stream_url), // 源流标称 分辨率/帧率/码率/编码（与本地无关）
  };
}
...
function buildInPageFetchScript(webRid) {
  ...
  return `(async () => {
    try {
      ...
      const to = setTimeout(() => ctrl.abort(), 8000); // 防止个别房间接口卡死
...
async function resolveStream(ctx, roomUrlOrRid, opts = {}) {
  ...
  // 仅当接口没查明状态(unknown)时才兜底；接口已明确「没在播(ended)」就直接返回，不浪费导航排队
  if ((!result || (!result.ok && result.status === 'unknown')) && typeof ctx.withNav === 'function') {
    try {
